...
class PageView:
    """Horizontal swipable pages. Each page is a list of widgets."""

    def __init__(self, display, touch, theme=None):
        self.display = display
        self.touch = touch
        self.theme = Theme() if theme is None else theme
        self.pages = []
    ...
    def _display_size(self):
        ...
        for name in ("width", "height"):
            ...
            if value is None and _amoled is not None:
                value = getattr(_amoled, name.upper(), None)
            dimensions.append(int(value) if value is not None else 1)
    ...
    def draw(self):
        ...
        if self._page_changed:
            ...
            self.display.fill_rect(
                0, 0, width - 1, height - 1, self.theme.background
            )
            for widget in page:
                widget.invalidate()
        for widget in page:
    ...
    def _hit_test(self, page, x, y):
        for widget in reversed(page):
            if widget.visible and widget.enabled and widget.contains(x, y):
                return widget
        return None

    def _has_dirty_widgets(self):
        if not self.pages:
            return False
        for widget in self.pages[self.page_index]:
            if widget.visible and widget.dirty:
                return True
        return False

    def _is_scroll_target(self, widget):
        while widget is not None:
            if isinstance(widget, ScrollView):
                return True
            widget = getattr(widget, "_scroll_parent", None)
        return False

    def _consumes_touch(self, widget):
        if widget is None:
            return False
        if isinstance(widget, ScrollView) or self._is_scroll_target(widget):
            return True
        widget_type = type(widget)
        return any(
            getattr(widget_type, name, None) is not getattr(Widget, name)
            for name in ("pointer_down", "pointer_move", "pointer_up")
        )

    def _dispatch(self, method, x, y):
        if self._active_widget is None:
            return
        getattr(self._active_widget, method)(x, y)
        if self._active_widget.dirty:
            self._dirty = True

    def _begin_touch(self, x, y):
        self._start_x, self._start_y = x, y
        page = self.pages[self.page_index] if self.pages else []
        self._active_widget = self._hit_test(page, x, y)
        self._gesture_consumed = self._consumes_touch(self._active_widget)
        self._dispatch("pointer_down", x, y)

    def _release_touch(self, x, y):
        self._dispatch("pointer_up", x, y)
        self._active_widget = None

    def _finish_swipe(self):
        if self._start_x is None:
            return
        dx = self._last_x - self._start_x
        dy = self._last_y - self._start_y
        old_page = self.page_index
        if (
            not self._gesture_consumed
            and abs(dx) > self.swipe_threshold
            and abs(dx) > abs(dy)
        ):
            if dx < 0 and self.page_index + 1 < len(self.pages):
                self.page_index += 1
            elif dx > 0 and self.page_index > 0:
                self.page_index -= 1
        self._start_x = None
        self._gesture_consumed = False
    ...
    def update(self):
        now = _ticks_ms()
        home = getattr(self.touch, "home", None)
        home_down = bool(callable(home) and home())
        home_ready = self._last_home is None or _ticks_diff(now, self._last_home) >= 500
        if home_down and not self._home_down and home_ready:
            self._last_home = now
            ...
            if self._screen_on:
                ...
                self.display.sleep()
            self._release_touch(self._last_x, self._last_y)
            self._start_x = None
            self._gesture_consumed = False
        self._home_down = home_down
        ...
        if point is None:
            ...
            if event == EVENT_DOWN:
                if self._active_widget is not None:
                    self._dispatch("pointer_up", self._last_x, self._last_y)
                    self._active_widget = None
                self._start_x = None
                self._gesture_consumed = False
                self._begin_touch(x, y)
            elif self._start_x is None:
                pass
            else:
                self._last_x, self._last_y = x, y
                if event == EVENT_CONTACT:
                    self._dispatch("pointer_move", x, y)
                elif event == EVENT_UP:
                    self._release_touch(x, y)
                    self._finish_swipe()
        if self._has_dirty_widgets():
            self._dirty = True
        if self._dirty:
            self.draw()
        return point is not None
